...
class FaissStickerVectorBackend(StickerVectorBackend):
    backend_type = "faiss"
    SCHEMA_VERSION = 1
    ...
    async def rebuild_full(
        ...
        db_ids = await self.document_storage.insert_documents_batch(
            doc_ids, texts, metadatas
        )

        vectors = np.array(
            [
                self._coerce_entry_vector(entry, embedding_dimension)
    ...
    async def upsert(
        ...
        new_db_id = await self.document_storage.insert_document(
            sticker_id, text, metadata
        )
        coerced = np.array(
    ...
    async def search(
        ...
        for distance, raw_id in zip(
            distances[0].tolist(), indices[0].tolist(), strict=False
        ):
            db_id = int(raw_id)
            if db_id < 0:
                continue
